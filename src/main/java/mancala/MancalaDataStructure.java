...
public class MancalaDataStructure implements Serializable {
    ...
    public final int PITS_PER_PLAYER = 6;
    ...
    public boolean isStore(int pit) {
        return pit == 6 || pit == 13;
    }
    ...
    public int getNextPitIndex(int currentPit, int currentPlayer) {
        int nextPit = currentPit + 1;
    
        if (nextPit > 13) {  // Assuming TOTAL_PITS is the total number of pits excluding stores
            nextPit = 1; // Wrap around to the start
        }
    
        if (nextPit == currentPit) {
            nextPit++; // Skip the starting pit
            if (nextPit > 13) {
                nextPit = 1; // Wrap around again if needed
            }
        }
    ...
    public void resetPitsAndStores() {
        setUpPits();
    ...
    public void setUpPits() {
        ...
        /**
     * Gets the store for a specified player.
     *
     * @param playerNum The player number (1 or 2).
     * @return The {@link Store} object for the specified player.
     */
    public Store getStore(int playerNum) {        
        return (Store) data.get(storePos(playerNum));
    }
    /**
